...
from app.repositories import (TrainingSessionRepository, StaffRepository,
                              ServiceRepository, ClientRepository)


class ClientTrainingsDialog(QDialog):
    """Окно со списком тренировок конкретного клиента."""

    def __init__(self, session_factory, client_id, client_name, parent=None):
        super().__init__(parent)
        self.session_factory = session_factory
        self.client_id = client_id
        self.setWindowTitle(f"Тренировки клиента: {client_name}")
        self.resize(750, 500)
        ...
        self.table.setHorizontalHeaderLabels([
            ...
            "Тренер", "Услуга", "Статус", "Действия"
        ])

        # Узкие колонки
        self.table.setColumnWidth(0, 55)  # ID
        self.table.setColumnWidth(1, 140)  # Начало
        self.table.setColumnWidth(2, 140)  # Конец
        self.table.setColumnWidth(5, 80)  # Статус
        self.table.setColumnWidth(6, 180)  # Действия

        # Растягиваемые колонки
        self.table.horizontalHeader().setSectionResizeMode(
            3, QHeaderView.ResizeMode.Stretch  # Тренер
        )
        self.table.horizontalHeader().setSectionResizeMode(
            4, QHeaderView.ResizeMode.Stretch  # Услуга
        )
    ...
    def load_data(self):
        ...
        try:
            trainings = TrainingSessionRepository.get_by_client(session, self.client_id)
    ...
    def add_training(self):
        dialog = TrainingEditDialog(
            self.session_factory, self.client_id, parent=self
        )
    ...
    def edit_training(self, training_id):
        dialog = TrainingEditDialog(
            self.session_factory, self.client_id, training_id=training_id, parent=self
        )
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.load_data()
...
class TrainingEditDialog(QDialog):
    ...
    def __init__(self, session_factory, client_id, training_id=None, parent=None):
        super().__init__(parent)
        self.session_factory = session_factory
        self.client_id = client_id
        ...
        # Имя клиента (только чтение)
        session = self.session_factory()
        try:
            client = ClientRepository.get_by_id(session, client_id)
            client_name = client.full_name if client else f"ID {client_id}"
        finally:
            session.close()
    ...
    def save(self):
        ...
        if not staff_id or not service_id:
            QMessageBox.warning(self, "Ошибка", "Выберите тренера и услугу.")
            return
